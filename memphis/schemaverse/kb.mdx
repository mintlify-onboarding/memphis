...
When attaching a schema, the producer is forced to pass the format that used in the schema creation, for example [protobuf](/memphis/memphis/schemaverse-schema-management/getting-started), and therefore we remove the `Buffer.from(<string>)`and use (example in JS) -
