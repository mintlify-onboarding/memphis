...
4. The producer will treat the lack of acknowledgment as a temporary network
   issue and retry sending the message (since it can’t know it was received).

5. In that case, the broker will end up having the same message twice.

![](/memphis/concepts/images/idempotence%201.jpeg)
...
As explained in
"[How does it work internally?](/memphis/memphis/concepts/idempotency#how-does-it-work-internally)",
...
automatically in the [DLS](/memphis/memphis/concepts/dead-letter) for supervised
retries.
