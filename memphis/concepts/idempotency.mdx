...
4. 4.  
Based on `maxAckTimeMs` parameter Memphis broker decides to retransmit the same message

5. 5.  
The consumer will process the same message again

![](/images/idempotence%202.jpeg)
...
![](/images/idempotence%20producer.jpeg)
...
![](/images/Screen%20Shot%202022-11-30%20at%2012.18.38.png)
...
As explained in "[How does it work internally?](/memphis/memphis/concepts/idempotency#how-does-it-work-internally)", the timer is responsible for the retention of the messages IDs table. For example, if 3 hours are chosen, the table will be emptied every three hours.
...
By configuring `maxMsgDeliveries` to 1, in a sudden failure of the consumer in a CG, the entire CG will not receive the same message again, and it will be stored automatically in the [DLS](/memphis/memphis/concepts/dead-letter) for supervised retries.
