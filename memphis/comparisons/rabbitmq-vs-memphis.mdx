...
![](/images/memphis%20vs%20legacy.jpeg)
...
Memphis uses a pull-based architecture where consumers pull messages from the server, and [long-polling](https://en.wikipedia.org/wiki/Push%5Ftechnology#Long%5Fpolling) is used to ensure new messages are made available instantaneously.
...
Memphis uses a distributed commit log called streams (made by NATS Jetstream) as its storage layer, which can be written entirely on the broker's (server) memory or disk. Memphis abstracts the offsets by default, so saving a record of the used offsets resides on Memphis and not on the client.
...
![](/images/Screen%20Shot%202022-12-22%20at%2014.10.16.png)
...
![](/images/Screen%20Shot%202022-12-28%20at%2015.18.45.png)
