...
$# mem connect Use command: mem connect --user \--password \--server Example:
mem connect -u root -p memphis -s http://:Connection configuration to Memphis
control plane Usage: index connect \[options\] Connection to Memphis control
plane Options: -u, --user User -p, --password Password -s, --server Memphis
